...
num_epoches, start_decline_epochs =101, 30
weight_decay = 1e-4
batch_size = 512
k = 5
...
class EEGDataset(Dataset):
    def __init__(self, samples, labels, transform=True, target_transform=None):
...
class ResnetModel(nn.Module):
    def __init__(self):
        ...
        self.model = ResNetPlus(63, c_out = 128)
...
if __name__ == '__main__':
    ...
    for patient in range(2,6):
        ...
        train_split = [i for i, x in enumerate(file_names) if x < 200]
        test_split = [i for i, x in enumerate(file_names) if x > 200]
        ...
        # mean = 0.0
        # std = 1e-5
        # ee = train_samples + np.random.normal(mean, std, train_samples.shape)
        # # 创建一个2x1的子图
        # fig, (ax1, ax2) = plt.subplots(2, 1)
        # ax1.plot(train_samples[0][0])
        # ax2.plot(ee[0][0])
        # plt.show()
        ...
        model_path = orin_data_path+'/model_file'+'/resnet'
